...
/**
 * Pure-JS CryptoProvider for React Native / Hermes (no crypto.subtle). @noble
 * supplies HKDF / HMAC-SHA256 / AES-GCM; expo-crypto supplies the CSPRNG. Verified
 * byte-for-byte compatible with WebCrypto (see the shared crypto suite + the
 * standalone interop check), so hub, companion, and any WebCrypto client interop.
 */
